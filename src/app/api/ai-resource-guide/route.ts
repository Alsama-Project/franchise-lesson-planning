...
/**
 * GET /api/ai-resource-guide
 *
 * Admin-only download of the guide CURRENTLY IN FORCE — the derived `content`
 * text (markdown) that the resource generator actually consumes, not the
 * original uploaded file (the bytes are not retained; original-byte retention is
 * a separate branch). Served as a `.md` attachment.
 *
...
export async function GET() {
  ...
  // Active version = latest created_at (mirrors get_active_resource_guide()).
  const supabase = await createClient();
  const { data, error } = await supabase
    .from('ai_resource_guide')
  ...
  const filename = deriveMarkdownFilename({
    ...
    fallbackSlug: 'ai-resource-guide',
