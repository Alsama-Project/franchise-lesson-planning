 ...
 * Admin → Centres · Subjects · Classes · Calendar · Curriculum · AI instructions ·
 * Users (+ Profile first). The admin Members tab is retired — user access is
 * managed from the Users-tab "Edit access" modal. Coordinator (non-admin, ≥1
 * coordinated subject) → Members · Curriculum. Everyone has Profile. Teacher →
 * Profile only. (The per-subject worksheet scaffold moved to the AI-instructions
 * board as a `worksheet_builder` document, so there is no Worksheet Templates tab.)
 */
export async function getConsoleAccess(): Promise<ConsoleAccess> {
  ...
  if (isAdmin) {
    tabs.push('centres', 'subjects', 'classes', 'calendar', 'curriculum', 'ai_instructions', 'users');
    defaultTab = 'centres';
